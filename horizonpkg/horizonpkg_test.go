...
func Test_DockerImagePkgBuilder_Suite(t *testing.T) {
	...
	builder, err := NewDockerImagePkgBuilder(FILE, author, specVer)
	if err != nil {
	...
	t.Run("DockerImagePkgBuilder.AddPart() permits empty signatures for part when builder is so configured", func(t *testing.T) {
		unsecureBuilder, _ := NewDockerImagePkgBuilder(FILE, author, specVer)
		unsecureBuilder.SetPermitEmptySignatures()
		_, err := unsecureBuilder.AddPart("", "1234567890123456789012345678901234567890", "someimage:latest", []string{}, 33, PartSource{"https://goo.foo"})
